...
Define_Module(PgMacLayer);

void PgMacLayer::initialize(int stage)
{
	coreEV << "PgMac init stage: " << stage << endl;
 ...
 */
void PgMacLayer::handleUpperMsg(cMessage *msg) {
	...
	double txPwr = dynamic_cast<PgNetwToMacControlInfo*>(cInfo)->getTxPower();
	debugEV<<"#####PgMac#### CSMA received a message from upper layer, name is " << msg->getName() <<", CInfo removed, mac addr="<< getUpperDestinationFromControlInfo(cInfo) << ", txPower="<< txPwr << endl;
...
void PgMacLayer::updateStatusIdle(t_mac_event event, cMessage *msg) {
	...
	case EV_SEND_REQUEST:
		if (macQueue.size() <= queueLength) {
			...
			//std::cerr << "starting timer_backoff. queue length " << macQueue.size() << std::endl;
		} else {
...
void PgMacLayer::updateStatusBackoff(t_mac_event event, cMessage *msg) {
...
void PgMacLayer::attachSignal(MacPkt* mac, simtime_t_cref startTime) {
	cObject *const cInfo = mac->removeControlInfo();
...
void PgMacLayer::updateStatusCCA(t_mac_event event, cMessage *msg)
{
	switch (event) {
	case EV_TIMER_CCA:
	{
		...
		if(isIdle) {
			...
			mac->setControlInfo(static_cast<MacPkt *>(macQueue.front())->getControlInfo());
...
void PgMacLayer::updateStatusWaitAck(t_mac_event event, cMessage *msg) {
...
void PgMacLayer::manageMissingAck(t_mac_event event, cMessage *msg) {
...
void PgMacLayer::updateStatusSIFS(t_mac_event event, cMessage *msg) {
...
void PgMacLayer::updateStatusTransmitAck(t_mac_event event, cMessage *msg) {
...
void PgMacLayer::updateStatusNotIdle(cMessage *msg) {
 ...
 */
void PgMacLayer::executeMac(t_mac_event event, cMessage *msg) {
...
void PgMacLayer::updateMacState(t_mac_states newMacState) {
 ...
 */
void PgMacLayer::fsmError(t_mac_event event, cMessage *msg) {
	EV<< "FSM Error ! In state " << macState << ", received unknown event:" << event << "." << endl;
	if (msg != NULL)
	delete msg;
}

void PgMacLayer::startTimer(t_mac_timer timer) {
...
simtime_t PgMacLayer::scheduleBackoff() {
 ...
 */
void PgMacLayer::handleSelfMsg(cMessage *msg) {
 ...
 */
void PgMacLayer::handleLowerMsg(cMessage *msg) {
...
void PgMacLayer::handleLowerControl(cMessage *msg) {
...
/* TODO poroblem might be DeciderResult802154Narrow related */

cPacket *PgMacLayer::decapsMsg(MacPkt * macPkt) {
